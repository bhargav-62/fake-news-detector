...
class NewsDataset(Dataset):
    def __init__(self, dataframe, text_col="text", label_col="label", max_len=256, tokenizer_name="distilbert-base-uncased"):
        self.texts = dataframe[text_col].astype(str).tolist()
        # Convert labels to numeric: "FAKE"->0, "REAL"->1
        self.labels = dataframe[label_col].map({"FAKE":0, "REAL":1}).tolist()
...
def eval_one_epoch(model, loader, device):
    ...
    acc = accuracy_score(labels, preds)
    report = classification_report(labels, preds, target_names=["FAKE", "REAL"], zero_division=0)
...
def train_model(train_df, val_df, model_name="distilbert-base-uncased", epochs=2, lr=2e-5, batch_size=16, save_dir="best_fake_news_model"):
    ...
    train_dataset = NewsDataset(train_df, text_col="text", label_col="label", tokenizer_name=model_name)
    val_dataset   = NewsDataset(val_df, text_col="text", label_col="label", tokenizer_name=model_name)
...
if __name__ == "__main__":
    ...
    train_df, val_df = load_data("news_dataset.csv")
    # Train model
    train_model(train_df, val_df, epochs=3, batch_size=8)
